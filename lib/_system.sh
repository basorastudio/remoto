...
system_set_timezone() {
  ...
  # Define o timezone padrão como America/Santo_Domingo, se não for passado um valor
...
criar_banco_dados() {
  ...
  sudo su - root <<EOF
docker run --name postgresql-${nome_instancia} -e POSTGRES_USER=izing -e POSTGRES_PASSWORD=${pg_pass} -e TZ="America/Santo_Domingo" -p ${porta_postgre_intancia}:5432 --restart=always -v /data:/var/lib/postgresql/data -d postgres
